...
namespace dawn {
    ...
    template<typename T__> using Sp_from_this = std::enable_shared_from_this<T__>;

# define ptr_sCast(T__, v__) std::static_pointer_cast<T__>(v__)
# define ptr_dCast(T__, v__) std::dynamic_pointer_cast<T__>(v__)

//    template<typename T__> using Bsp = boost::shared_ptr<T__>;
//    template<typename T__> using Bwp = boost::weak_ptr<T__>;
//    template<typename T__> using Bsp_from_this = boost::enable_shared_from_this<T__>;
//
//# define bptr_sCast(T__, v__) boost::static_pointer_cast<T__>(v__)
//# define bptr_dCast(T__, v__) boost::dynamic_pointer_cast<T__>(v__)
    ...
    */
    ...
    public:
        ...
        template<typename CT__>
        inL Array_general(std::deque<CT__>& deque_)
            : value_count(static_cast<type_of_size__>(deque_.size()))
            , first_value_ptr(deque_.empty() ? nullptr : const_cast<const_godie_t<T__>*>(&deque_.front()))
        {   }
